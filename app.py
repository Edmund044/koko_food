...
class TransactionsModel(db.Model):
    ...
    def createInvoices():    
         invoice_created_query_results = db.session.query(TransactionsModel.id,TransactionsModel.meal_id,TransactionsModel.time_created,func.count(TransactionsModel.meal_id).label('served_plates'),TransactionsModel.time_created).filter(TransactionsModel.status == False).group_by(func.strftime('%m', TransactionsModel.time_created))
         invoices_created = [{
